...
use App\Telegram\Entities\Chat;
use App\Telegram\Entities\ChatJoinRequest;
use App\Telegram\Entities\ChatMemberUpdated;
use App\Telegram\Entities\Message;
use App\Telegram\Entities\Update;
use App\Telegram\Entities\User;
use App\Telegram\Models\TelegramCallbackQuery;
use App\Telegram\Models\TelegramChat;
use App\Telegram\Models\TelegramChatJoinRequest;
...
class DB
{
    ...
    static public function setExpectation(?int $user_id = null, $expectation = null): void
    {
        ...
        // self::$telegram::sendMessage(json_encode(self::getUser($user_id)), null, self::$telegram->getMainAdmin());
    }
    ...
    static public function getLastMessageId(?int $chat_id = null): ?int
    {
        return TelegramMessage::where([
            ...
            'from'  => self::getUser(self::getBot()->bot_id)->id
        ])->get()?->last()?->message_id;
    }
    ...
    static protected function getBot(): Bot
    {
        $bot = Bot::where('bot_id', self::$telegram->getBotId())->first();

        if (!$bot) {
            throw new TelegramException('Bot not found');
        }
